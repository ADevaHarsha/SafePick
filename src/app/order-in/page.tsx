...
import { PackagePlus, CheckCircle2, QrCode, ClipboardList, MapPin, Hash, Phone, User, Send } from "lucide-react";
...
/**
 * Generates a unique institutional order ID
 * Format: SP-[Timestamp(Base36)]-[Random(Base36)]
 */
...
export default function OrderInPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast({
      ...
      description: `Sent to ${formData.phoneNumber}: "SafePick Alert: ${formData.receiverName}, your parcel '${formData.description}' is stored at Rack ${formData.rackNumber}. Use ID: ${id} for collection."`,
    });
  };
          ...
          <CardContent className="p-8 grid md:grid-cols-2 gap-8 items-center">
            <div className="space-y-6">
              <div className="space-y-4">
                ...
                <div className="p-4 bg-white/5 rounded-lg border border-white/10">
                  ...
                  <p className="text-lg font-medium">{formData.location} - Rack {formData.rackNumber}</p>
                </div>
              </div>
              <Button onClick={() => setIsSubmitted(false)} variant="outline" className="w-full border-primary/20 hover:bg-primary/5">
                Register New Order
              </Button>
            </div>
            ...
            <div className="flex flex-col items-center justify-center p-8 bg-white/5 rounded-2xl border border-white/10 glow-orange">
              <div className="relative h-48 w-48 bg-white p-2 rounded-xl mb-4">
                <Image 
                  src={`https://picsum.photos/seed/${orderId}/400/400`} 
                  alt="Unique QR Code" 
                  width={400} 
                  height={400} 
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">Register new intake. A unique ID and QR will be generated and sent to the receiver.</p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end gap-4 items-center">
              <p className="text-xs text-muted-foreground italic">
                A secure ID and QR code will be generated upon submission.
              </p>
              <Button 
                type="submit" 
                size="lg" 
